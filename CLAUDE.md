...
The Flashcard screen has two study modes of its own: **Standard** (browse, records nothing) and **Mastery** (the spaced drill). Mastery is a *single-session* mode: three Know Its **in a row** master a card, each hit dropping it further down the rotation (5 / 10 / 15 cards, student-configurable via the gear, stored per device). A card's first-ever Know It jumps straight to 2/3, so known material clears in two passes; a miss drops it 3 and resets the streak to 0. Mastery itself persists per deck, so a later round only offers what isn't mastered yet — a fully mastered deck opens on "Deck Mastered", whose only real action is Reset Progress. The rotation never shrinks (mastered cards keep circulating as spacers), so a round ends on every card being mastered, not on the queue emptying.
