...
`test-decks/` has working example decks (quiz and flashcard) to test against. `prompts/` holds the AI-facing prompt/spec markdown, `?raw`-imported at build time by `src/lib/formatSpec.ts` so nothing drifts: `studydeck-format-spec.md` is the shared schema contract for generating new decks (both deck types, LaTeX/graph rules, question-quality bar, validation checklist) — it holds no conversational behavior. The Home screen's "Copy Prompt" button offers two variants: `QUICK_PROMPT_MD` = `studydeck-quick-intro.md` + the shared contract, `GUIDED_PROMPT_MD` = `studydeck-guided-intro.md` + the shared contract. Quick is today's fast, low-friction flow; Guided always confirms purpose/scope/deck-type before generating (capped at one follow-up round) so the resulting questions are better-targeted. Update the shared contract alongside `design-doc.md` whenever the schema changes; update the intro files when conversational behavior changes. `prompts/studydeck-quiz-spec.md` and `studydeck-flashcard-spec.md` are a per-deck-type split of the shared contract (goal: ~10k characters per composed prompt, including the intro) — drafted but **not yet wired into `formatSpec.ts` or the UI**; the quiz half still needs real trimming to hit that budget before it's adopted.
