...
The Flashcard screen has two study modes of its own: **Standard** (browse, records nothing) and **Mastery** (the spaced ladder). Mastery is a *come-back-tomorrow* mode by design — three spaced hits in one session make a card provisional, and a cold check on a later day is what actually masters it. Mastered cards then resurface as occasional *refreshers* (3/7/16/35 days, at most three per session, spread through the ladder rather than run first); missing one un-masters the card outright. Mastery will still legitimately open onto an "All Caught Up" screen with nothing to do; that's the mode working, not a bug. See the design doc's "Mastery ladder" section before changing any of it. Practice shows live feedback with retries (retries don't affect stats); Test shows no feedback until Stats and disallows retries; Review is a read-only browser with the correct answer shown.
