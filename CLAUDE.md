...
- **`src/lib/fillBlank.ts`** — the `___` marker syntax for `fillBlank` questions, defined in exactly one place: `parseBlanks` splits a sentence into text runs and blank slots, `countBlanks` feeds validation. Runs of one or two underscores and any marker inside `$...$` stay text. Change the marker here or nowhere.
...
- **`src/features/share/`** — `shareLibrary.ts` owns "do they already have this deck?" (token match, then content hash — never title), adding one, and `isSharedCopy()`, the single expression of "this set is someone else's, so it's read-only here"; `shareSync.ts` owns the live half — `renameSharedDeck()` (the ONLY caller of `Storage.renameFile`) and the once-per-session pull that brings every shared copy to its owner's current name; `ShareModal.tsx` mints/copies a link from a Home deck card; `ShareScreen.tsx` is the landing a link opens.
...
- **`src/components/QuizUI.tsx`** — the presentational shell shared by Quiz and Review (`ProgressHeader`, `QuestionBody`, `AnswerList`, plus the `letterFor()` answer-label helper). Deliberately shell-only: Quiz's scoring/retry-locking state and Review's static correct-highlight stay in their own screens, passed in as per-button className/disabled/onSelect callbacks. Don't grow this into a merged Quiz+Review component — the two modes' logic is genuinely different, only the markup isn't. `fillBlank` is the one format whose answer surface lives *inside* the question sentence: `QuestionBody` takes an optional `blankSlots` prop and renders `FillBlankText` instead of plain question text. Same shell-only split as everything else here — the caller still owns the values, grading and per-blank classes.
- **`src/lib/answerMatching.ts`** — the free-response matchers: `matchNumeric`, `matchNormalizedString`, and `matchBlankText` (fill-in-the-blank; case/whitespace/curly-apostrophe/trailing-punctuation forgiving, deliberately NOT fuzzy — real alternates go in the deck's `accept` array, never in an edit-distance threshold).
...
- Quiz questions: stable string `id`, `question` (LaTeX via `$...$`/`$$...$$`), 2 or more `answers` (4 is conventional, not required — relaxed 2026-09-06), integer `correct` indexing one of them, optional `graph` (`points` or `equation`). A non-`mcq` `answerFormat` swaps `answers`/`correct` for its own fields — `fillBlank` in particular has neither, carrying `___` markers in `question` plus a positional `blanks[]` answer key whose length must match the marker count.
...
- History entries may also carry `shareToken` — set on a deck the student published AND on every copy added from that link. It is the dedupe key for share links and is preserved across saves exactly like `id`. `shareOwner` is the companion flag that tells those two apart: true only on the deck this student published. Both are preserved across saves; absence of `shareOwner` reads as read-only, and a login settles it from `shared_decks.owner_id`.
...
**The sharer owns the set** (Davis, 2026-09-14). A recipient's copy is read-only — no rename, no pencil in the Home row, just a brass "Shared with you" marking where the control would be — and its name tracks the owner's: renaming a published set reaches every copy, **silently, on the recipient's next app open**. No badge, no prompt, no accept step; Davis was asked and chose the quiet version. Progress is untouched by an update because the rename goes through `Storage.renameFile`, which keeps the deck's `id` (mastery and quiz-order are id-keyed) and moves the title-keyed cloud rows rather than stranding them. Questions still can't be edited anywhere, so a rename is currently the only thing that propagates — `shareSync.ts` is the seam a future question edit plugs into, and it already pulls each snapshot's content hash for that purpose. **Don't add an "update available" prompt or a recipient-side fork without asking.**
...
**Renaming goes through `shareSync.renameSharedDeck`, which is the only thing that may call `Storage.renameFile`.** `saveFile` upserts *by title*, so saving an entry under a new title would add a second deck instead of renaming the first. `renameFile` rewrites history directly (id, `shareToken`, `shareOwner` and `data` all preserved — `data.title` is deliberately left alone, so a shared copy's content hash and the link dedupe survive) and mirrors to the cloud as save-new-title → delete-old-title for BOTH title-keyed tables, `decks` and `flash_state`. It refuses a title another deck already holds; the UI says so under the field rather than freeing a "(2)" name. `renameSharedDeck` adds the two share rules on top: it refuses a copy of someone else's set, and on a set the student published it mirrors the new name to `shared_decks` via `rename_share` so every recipient gets it.
...
- `shared_decks` must never get a blanket public `select` policy; token reads go through the `get_shared_deck` / `get_shared_titles` SECURITY DEFINER functions, or the anon key becomes a directory of everyone's shared decks. It has no `update` policy either — `rename_share` is the only write into an existing row, and it may touch `title` and nothing else, so a rename can never move a recipient's content hash.
- A recipient's copy of a shared set is read-only. Gate on `isSharedCopy()`, never on `share_token` alone — the token is stamped on the owner's deck too.
