...
Before writing or modifying code in a feature, invoke `requirements-writer` (before-pass) scoped to that feature, passing the invocation contract: pass, feature, intent, planned files. **Do NOT run the after-pass by default** — it burns too many credits (Davis's call, 2026-07-10). Instead, the implementing thread self-verifies against the updated spec's R-IDs before committing. Run an after-pass only when Davis explicitly asks for an audit, or for changes that touch the fragile `[caution:]`-tagged invariants (first-attempt scoring, id-keyed piles, Storage atomicity/quota, graph never-throws). If an after-pass does run and reports gaps: fix, re-invoke, max 3 rounds, then surface to Davis.
