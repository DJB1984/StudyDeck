...
`test-decks/` has working example decks (quiz and flashcard) to test against. `prompts/` holds the AI-facing prompt/spec markdown, `?raw`-imported at build time by `src/lib/formatSpec.ts` so nothing drifts: `studydeck-quiz-spec.md` and `studydeck-flashcard-spec.md` are the per-deck-type schema contracts (LaTeX rules, question-quality bar, validation checklist) — they hold no conversational behavior. The Home screen's "Copy Prompt" button opens a modal with **four** options — Quick Quiz, Quick Flashcards, Guided Quiz, Guided Flashcards — since deck type is now chosen by which card the student clicks, not inferred mid-conversation. Each composes one deck-type intro with its matching single-deck-type contract: `QUICK_QUIZ_PROMPT_MD` = `studydeck-quick-quiz-intro.md` + `studydeck-quiz-spec.md`, `QUICK_FLASHCARD_PROMPT_MD` = `studydeck-quick-flashcard-intro.md` + `studydeck-flashcard-spec.md`, `GUIDED_QUIZ_PROMPT_MD` = `studydeck-guided-quiz-intro.md` + `studydeck-quiz-spec.md`, `GUIDED_FLASHCARD_PROMPT_MD` = `studydeck-guided-flashcard-intro.md` + `studydeck-flashcard-spec.md`. Quick is today's fast, low-friction flow; Guided always confirms purpose/scope before generating (capped at one follow-up round, no longer asks quiz-vs-flashcard since the button already answered that) so the resulting questions are better-targeted. Update the contracts alongside `design-doc.md` whenever the schema changes; update the four intro files when conversational behavior changes — quiz and flashcard variants of each mode should stay in sync in tone/structure even though their content differs.

`studydeck-quiz-spec.md` documents `mcq` (the default answer format) inline and nothing else — the other answer formats (`multiSelect`, `numeric`, `order`, `code`) plus the `graph`/`table` context add-ons are each a separate fetchable addendum under `public/formats/` (`multiselect.md`, `numeric.md`, `order.md`, `code.md`, `graph.md`, `table.md`), served as plain static files at `studydeck.brookslanding.com/formats/<name>.md` once deployed. The quiz spec's own "Other answer formats" catalog tells the AI to fetch the matching one only if the material calls for it, and to fall back to plain `mcq` if it can't fetch — so decks that only need mcq get a smaller prompt, and non-mcq formats only cost prompt size when actually used. **Caveat:** `public/formats/` hasn't been deployed yet as of this writing, so those URLs currently 404 on the live site; until the next deploy, any AI that tries to fetch one will fail and (per its own fallback instruction) should degrade to `mcq` rather than guessing at the schema. `prompts/studydeck-format-spec.md` is the original single-file contract (both deck types, all answer formats inlined, no fetch dependency) — kept in the repo as a fallback/reference but **not imported by `formatSpec.ts`**; nothing currently wires it into the UI.
