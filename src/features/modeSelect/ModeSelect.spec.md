...
- R4 [verify: ui] On entering the screen, selection resets to `practice`. [caution: this must reset on every entry — a stale selection from a previous deck must not carry over.]
- R5 [verify: ui] Clicking a mode card selects it (visually distinct selected state) and deselects the others.
- R7 [verify: ui] Start launches the selected mode with the current deck's questions, always in the natural `0..n-1` order. [caution: there is deliberately no random-order option here — Davis removed it as unhelpful (2026-07-21). Do not re-add it. The Flashcard screen's own random toggle is unrelated and out of scope — see Flashcard spec R7/R12.]
...
- 2026-07-21: Removed the random-order toggle (R6 deleted, R4/R7 updated) — Davis: "Remove the random order slider. It's not helpful." Question order is now unconditionally natural `0..n-1`; `QuizSession.wasRandom` and the Stats retake random-reshuffle prompt were removed to match (see Stats.spec.md).
