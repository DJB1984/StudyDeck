...
- **Fill-in-the-blank sentences** *(added 2026-09-14)*. A sentence with one or more terms missing, typed into blanks that sit in the line of type rather than in a separate answer block. Tests recall of the term in the context that gives it meaning, which multiple choice can't do — a distractor list is a hint. Graded all-or-nothing per question, forgiving about case, whitespace and a trailing period but not about typos. No word bank in v1: offering the words turns recall back into recognition, and it can arrive later as its own opt-in flag (see Non-Goals).
...
- **Word bank for fill-in-the-blank.** A pool of candidate words (with distractors) the student taps into the blanks instead of typing. Deliberately deferred 2026-09-14: recall and recognition are different questions, and the typed version is the one worth having first. It can ship later as a per-question opt-in without reworking the format.
