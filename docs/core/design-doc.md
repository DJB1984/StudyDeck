...
- `answerFormat` — optional string discriminator, defaults to `"mcq"` (the shape above). One new field group per non-default value, all additive/optional so existing decks are unaffected:
  ...
  - `"fillBlank"` — a sentence with terms missing. `blanks: { accept: string[] }[]`, one entry per blank in reading order, required; optional `caseSensitive: boolean` (default false). Each blank is marked in `question` by a run of **three or more underscores**; `src/lib/fillBlank.ts` owns that syntax and is the only place it's defined, splitting the sentence into text runs and blank slots for rendering. Runs of one or two underscores (`snake_case`) and any marker inside `$...$` / `$$...$$` are left as text — a subscript chain must never read as a blank, and splitting mid-formula would hand KaTeX half a math span. Validation requires the marker count to equal `blanks`' length. Graded all-or-nothing via `answerMatching.matchBlankText`, which normalizes case (unless `caseSensitive`), collapses whitespace, straightens curly apostrophes and drops trailing sentence punctuation, and is deliberately NOT fuzzy — one character is often the whole distinction being tested, so real alternates go in `accept` instead. Rendered by `FillBlankText` in `QuizUI.tsx`, reached through `QuestionBody`'s optional `blankSlots` prop: the inputs live *inside* the question sentence, so unlike every other format there is no separate answer block below it. Practice highlights each blank individually and says "N of M blanks right" — which blank is wrong is already visible from the highlight, so the tally leaks nothing extra. Review passes each blank's first-accepted answer in read-only.
...
**Home screen:** Grid of file cards (title, question count, last opened date). Drag-drop zone at top. "Load file" button. Clicking a card goes to Mode Select. Each card's right margin carries three controls, drawn on one 24px grid at one stroke weight: a pencil (rename) and a link glyph (share), both accent on hover, then a hairline, then the × (remove — alert red on hover). **Rename is edited in place:** the pencil swaps the title for an input of the same size in the same position, Enter or blur commits, Escape cancels, and a name another set already holds is refused under the field rather than silently freed with a "(2)" suffix. It renames the history ENTRY only — `data.title` inside the deck is left as generated or published, since a shared copy's content hash is what a later click of the same link uses to recognize it. Renaming goes through `Storage.renameFile`, never `saveFile` (which upserts by title and would add a second deck); question editing still does not exist.
...
**Mode select screen:** Mode cards — Practice, Test, Review, Flashcard decks skip straight to Flashcard. Start button, with a **Random order** switch on the same row to its right. The switch slides in when Practice or Test is selected and slides away under Review, which always browses the deck in its written order (shuffling a read-through only makes the same pass harder to find your place in). It is per deck, not per person or per visit: off for a set never answered on, then remembered under that deck's id in `studydeck_quiz_order` — a vocabulary set wants shuffling and a worked-derivation set wants its order, so the choice belongs to the material. Device-local and never mirrored to Supabase (Davis, 2026-09-14). Off means the natural `0..n-1`; on means one `shuffleArray` per Start, so two rounds of the same deck differ. (An early version of this toggle was removed as unhelpful and restored on 2026-09-14 with the per-deck memory that was missing the first time.)
...
**Undo sits in the options row, not with the verdicts** (Mastery only, immediately after the mode pill). It is a correction rather than a choice, and putting it a thumb-width from Still Learning made a slip and its repair neighbours; it is also the one control a phone keeps, because it is the only one no gesture can replace — undo has no direction to be thrown in. Its position reads mode first, then the one action that mode affords, and it costs no layout shift: the pill is anchored left and the Settings wrap is pushed right by `margin-left: auto`, so undo appears and vanishes in the slack between them.

**Swipe (`src/features/flashcard/useCardSwipe.ts`).** The card is draggable by any pointer — finger, pen, or mouse — through one Pointer Events path; a desktop click-drag behaves exactly like a phone swipe, because a card that visibly moves under a mouse has to be one you can throw with it. **Right is forward in both modes**, matching the arrow keys: the next card in Standard, Know It in Mastery; left is the previous card / Still Learning. Mechanics: a gesture locks to an axis after 10px and vertical wins ties (so the page's own scroll starts on the same pixel it would anywhere else, backed by `touch-action: pan-y` on the card); commitment is 28% of the card's own width, floored at 72px and capped at 150px, or a flick (≥0.55 px/ms travelling ≥44px in the direction it is still moving); a direction with nothing behind it — Standard's first card, swiped left — moves at 22% resistance to a 44px stop and shows no cue. Releasing short of commitment springs home over 300ms. The click that follows every release is suppressed when the gesture dragged, so a sort never also flips the card. The hook owns motion and commitment only; what a direction *means* stays in `FlashcardScreen`.

**The swipe cue is Mastery-only.** As the drag travels, the card's own words fade out and the verdict fades in where they were, with a matching rim, an edge wash, and a glow — all driven by one `--swipe-p` custom property the gesture writes on `#flash-card`. **Know It takes the mode's own hue** (The Mode-Owns-Its-Screen Rule — the screen's accent already means "the good direction"); **Still Learning takes Alert Red**, the one thing allowed to override the mode inside this screen, because it warns about the card rather than theming the screen. Standard gets no cue at all: it records nothing, so a card turning red on the way to the previous card would promise a consequence the mode does not have — there the card simply follows the finger. A committed swipe carries the card off the edge it was thrown toward (`--exit-from` / `--exit-rot` seed the keyframes from wherever the finger let go, so the throw never snaps back to centre first). **A button gives the identical exit** — same direction per verdict, same arc, starting from centre because a press has no offset to carry — and in Mastery it blooms the same rim and verdict word over the flight (`--swipe-p` jumps to 1, which is where a released swipe leaves it). Still Learning used to shake in place instead; one verdict now has one motion whatever gave it. The flight stays inside the layout: the drag tilt caps at 6deg, the exit keyframes drop 7% and turn 5deg, and `html, body` clip horizontal overflow, so nothing about a sort can grow the page or flash a scrollbar mid-gesture.

**On touch devices the action buttons are hidden and the gesture is the only input**, per Davis 2026-08-19, at every screen size. The test is `(pointer: coarse)` — a live media query read in `FlashcardScreen` as `touch`, the same one that decides the hint's wording — so the question asked is what's pointing at the screen, not how wide it is. A tablet in landscape sorts by gesture; a desktop window dragged to phone width keeps its buttons, because a mouse has nothing to swipe with that a button doesn't do better. This replaced a `max-width: 560px` test on the same day, which got that second case backwards. Note the hybrids: `pointer: coarse` reports the *primary* pointer, so a touchscreen laptop with a trackpad and an iPad with a Magic Keyboard both read as fine and keep their buttons — deliberately, since nothing should vanish mid-round on a device the student may be pointing at. The buttons are visually hidden rather than `display: none` — a phone screen reader has no gesture to give a verdict with, so removing them from the accessibility tree would remove the mode with them. Settings grows a **Card buttons** switch on touch (and only there, since nowhere else are they hidden) to bring them back. Off by default, remembered per device (`studydeck_card_buttons`). Settings itself still drops to icon-only below 560px (keeping its `aria-label` and tooltip) — that one is genuinely a width problem: carrying the word, the three controls overflow the 342px inner column by a few pixels and wrap onto a second row. With the buttons gone the card's cap rises to 560px so it takes the height they were using. The hint line under the card names the gesture instead of the keys where touch is the primary pointer, and retires itself the first time a swipe commits (`studydeck_swipe_hint`, per device) — it teaches one thing, then gives the height back.

Bottom controls follow the mode. Standard shows only two icon-only arrow buttons (previous / next card) — it has no verdict to record, so the pile buttons would have nothing to write. Mastery shows Still Learning / Know It (Undo moved up to the options row — see above). Keyboard splits by axis in both modes: ↑/↓ flip the card (as do Space and Enter), while ←/→ move the round on — stepping between cards in Standard, and giving the verdict in Mastery (← Still Learning, → Know It, matching the button order). Undo has no key: binding a correction to the same axis as the verdicts is how one slip becomes two. The window handler ignores keys typed into an input, **except** the mode pill's radios — they keep focus after a click, so without that exception an arrow pressed after switching mode moved the pill's own selection instead of the card; `preventDefault` suppresses the radiogroup's built-in arrow navigation. Standard is `!masteryMode` (`FlashEngine.isBrowseMode()`) — no separate stored flag, so a resumed session and the mode pill can't disagree. Its cursor moves via `stepBack()` / `stepForward()`, which touch neither pile nor the undo stack; stepping off the last card ends the round.
...
// Which decks shuffle in Practice and Test, keyed by deck id. One map rather
// than a key per deck — the value is a single bit. Only `true` entries are
// stored, so absence is off and turning it back off drops the entry. Device
// preference, never mirrored; cleared wholesale by clearLocal, since every id
// it names is about to stop existing.
"studydeck_quiz_order": { "3f2b...": true }
...
4. **The round ends when every card in the working set is mastered.** A round's working set is the deck's *unmastered* cards; cards mastered in an earlier session are left out entirely, so a later round only offers what's left. A fully mastered deck opens on the Deck Mastered screen, whose only substantive action is **Reset Progress** (`resetProgress()`, the one thing that wipes streaks). The Round Complete screen additionally offers **Undo Last Card** whenever the round has a sort to take back: the verdict that masters the last card ends the round, and gating undo on `isComplete()` made that one verdict the only unreachable undo in the mode — with nothing to fall back on but a reset of the entire deck. `goBack()` restores the record along with the queue, so the round simply stops being complete.
...
- **Older state is back-filled on read** (`schedule.ts` `normalize`), per record rather than per deck, since a cloud merge can leave a deck holding a mix of shapes. Old two-array `known` ids and ladder-era `step` values both land on the streak they actually demonstrated: a single old Know It becomes 3/4 (what a first-attempt hit is worth now), ladder rungs map across one for one, and ladder `mastered` stays mastered. Records from the **three-hit** model (2026-08-18, the same day it was replaced) need one more distinction: `streak: 3` meant *mastered* there, so it is promoted to 4 when — and only when — the same id also appears in the state's derived `known`, which under the current model never holds a 3/4 card. Without that, every already-finished deck would reopen as a full round. State arriving from the **cloud** does not take this path at all: it has the same two-array shape but the opposite meaning for `known` (mastered, since `derivePiles` puts nothing else there), and the two are indistinguishable by inspection — so `migration.ts` rebuilds the records at the boundary via `schedule.ts` `fromPiles()`, which knows they came from the current model. Left to `normalize`, a deck mastered on one device would land on a second at 0 mastered and then mirror that emptied pile back up, destroying the account's record.

**Deletion lifecycle:** `Storage.deleteFile(title)` removes the entry from `studydeck_history` AND deletes the corresponding `studydeck_flash_{id}` key (plus any pre-migration `studydeck_flash_{title}` key) and the deck's `studydeck_quiz_order` entry, in one atomic operation. This keeps localStorage clean and prevents orphaned card data accumulating over time.
...
**Mastery records are device-local.** The `flash_state` table still has only `deck_title`, `known` and `learning` columns, so `setFlashState`'s mirror resolves the deck's title and sends the derived piles — the `cards` records don't leave the device. What survives the trip is exactly what the two columns can say: which cards are **mastered** (`known`) and which have been seen (`learning`). A partial streak — 1/4, 2/4, 3/4 — doesn't, so a second device picks those cards up at 0. Deliberate (Davis, 2026-08-17: get the mode's shape right in real use before freezing it into a schema). Closing this means adding `cards jsonb` + `schema_version` to `flash_state` and keeping the two pile columns written in parallel for a release.
