...
**The swipe cue is Mastery-only.** As the drag travels, the card's own words fade out and the verdict fades in where they were, with a matching rim, an edge wash, and a glow — all driven by one `--swipe-p` custom property the gesture writes on `#flash-card`. **Know It takes the mode's own hue** (The Mode-Owns-Its-Screen Rule — the screen's accent already means "the good direction"); **Still Learning takes Alert Red**, the one thing allowed to override the mode inside this screen, because it warns about the card rather than theming the screen. Standard gets no cue at all: it records nothing, so a card turning red on the way to the previous card would promise a consequence the mode does not have — there the card simply follows the finger. A committed swipe carries the card off the edge it was thrown toward (`--exit-from` / `--exit-rot` seed the keyframes from wherever the finger let go, so the throw never snaps back to centre first); a verdict given by *button* keeps the older vocabulary — Know It files right, Still Learning shakes in place.

**Below 560px the action buttons are hidden and the gesture is the only input**, per Davis 2026-08-19. Settings drops to icon-only at the same width (keeping its `aria-label` and tooltip) — carrying the word, the three controls overflow the 342px inner column by a few pixels and wrap onto a second row. They are visually hidden rather than `display: none` — a phone screen reader has no gesture to give a verdict with, so removing them from the accessibility tree would remove the mode with them. Settings grows a **Card buttons** switch at that width (and only there) to bring them back, for a desktop window dragged narrow: the layout there is a phone's but the hands aren't. Off by default, remembered per device (`studydeck_card_buttons`). With the buttons gone the card's cap rises to 560px so it takes the height they were using. The hint line under the card names the gesture instead of the keys where touch is the primary pointer, and retires itself the first time a swipe commits (`studydeck_swipe_hint`, per device) — it teaches one thing, then gives the height back.
...
- Card sort by button (Know It): slide right and down + fade; Still Learning: brief shake + return
